...
public class ExcelReader {
	...
	public int getDataSets(String sheetName, String dataFlag, String filePath) {
		ExcelUtils xls = new ExcelUtils(filePath);
		int flagRowNumber = 1;

		while (!xls.getCellData(sheetName, 0, flagRowNumber).equalsIgnoreCase(dataFlag)) {
			flagRowNumber++;
	...
	public JSONObject getTestData(String sheetName, String dataFlag, String filePath, int iteration) {
		...
		while (!xls.getCellData(sheetName, 0, dataStartRowNumber).equals("")) {
			...
			if (index == iteration) {
				while (!xls.getCellData(sheetName, colNumber, dataStartRowNumber).equals("")) {
					String data = xls.getCellData(sheetName, colNumber, dataStartRowNumber);
					String column = xls.getCellData(sheetName, colNumber, colStartRowNumber);
...
//				+ "\\src\\test\\resources\\projectJSONs\\XLS_Data\\TestExcelData.xlsx";
//		ExcelUtils xls = new ExcelUtils(filePath);
//
//		String sheetName = "Manage Stocks";
//		String dataFlag = "addfreshstock";
//		int iteration = 1;
//
//		int flagRowNumber = 1;
//
